...
- 初次尝试设置 `390x844` 时页面仍报告 `1280x720`，当时未把该结果记为移动视觉验收通过。

## 移动视口浏览器复验

- 2026-07-19 使用可验证的浏览器 viewport override 复验，登录页、仪表盘和客户需求页均报告 `innerWidth=390`、`innerHeight=844`，文档 `scrollWidth=390`，无横向溢出。
- 登录页在 390 px 宽度下账号、密码、语言和登录操作完整可用；登录后仪表盘标题、语言选择、移动导航入口、指标和近期订单均可访问。
- 客户需求页在 390 px 宽度下标题、工作区标签、主按钮、搜索、状态筛选、刷新和需求卡片按单列布局显示；截图确认文字和操作按钮未相互遮挡。
- 搜索 `REQ-2026-000009` 后页面显示“当前活跃 1 项”，DOM 只有 1 个 `article` 且包含目标编号。
- 新建客户需求抽屉在窄视口下具备 `dialog` 语义，客户、来源渠道、摘要、金额、日期和创建操作均可访问。浏览器重连后的共享行为复验确认关闭后 dialog 被移除，焦点恢复到“新建客户需求”；该焦点结论不单独归为 390 px 视觉证据。
- 验收期间临时允许 `http://127.0.0.1:43182` CORS 来源并启用合成账号；结束后已再次禁用该账号、恢复单一可信 CORS、重建 API 并确认 healthy，SSH 隧道已关闭。
...
- 本次仅为受控 UAT，不代表生产就绪。
- 登录页和售前工作台已完成桌面与 390x844 移动视口的真实浏览器交互检查；本次覆盖不替代真实手机、不同浏览器和辅助技术的人工签字验收。
