...
export function EngineeringDocumentWorkbench() {
  ...
  const [revisionTarget, setRevisionTarget] = useState<EngineeringDocument | null>(null);
  const [reviewTarget, setReviewTarget] = useState<EngineeringRevision | null>(null);
  ...
  const loadDocuments = useCallback(async () => {
    setLoading(true);
    try {
      const response = await api.get<EngineeringDocument[]>('/engineering-documents', {
  ...
  const runAction = async (revision: EngineeringRevision, action: 'submit' | 'release') => {
    setBusyKey(`${revision.id}:${action}`);
    try {
      await api.post(`/engineering-documents/revisions/${revision.id}/${action}`, {});
  ...
  const reviewRevision = async (decision: 'APPROVE' | 'REQUEST_CHANGES') => {
    ...
    try {
      await api.post(`/engineering-documents/revisions/${reviewTarget.id}/review`, {
        decision,
        comment: reviewComment.trim() || undefined,
      });
      toast.success(decision === 'APPROVE' ? '校审通过，等待批准' : '已退回设计修改');
  ...
  const releasedCount = documents.filter((document) => document.currentReleasedRevisionId).length;
  const pendingCount = documents.filter((document) =>
    ['PENDING_REVIEW', 'PENDING_APPROVAL'].includes(document.revisions[0]?.status ?? ''),
  ).length;
    ...
    <div className="space-y-5 p-4 sm:p-6 lg:p-8">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-slate-950">工程文档工作台</h1>
          <p className="mt-1 text-sm text-slate-500">
            图纸与规范按版本校审、批准、发布；已发布二进制不可覆盖。
          </p>
        </div>
        {can('engineeringDocument:create') ? (
          <Button icon={<FilePlus2 className="h-4 w-4" />} onClick={() => setCreateOpen(true)}>
            新建工程文档
          </Button>
        ) : null}
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <StatCard label="受控文档" value={documents.length} icon={FileText} tone="blue" />
        <StatCard label="已发布" value={releasedCount} icon={ShieldCheck} tone="green" />
        <StatCard label="待处理" value={pendingCount} icon={History} tone="indigo" />
      </div>
      ...
      <Sheet open={createOpen} title="新建工程文档" onClose={() => setCreateOpen(false)}>
        <div className="space-y-4">
          ...
          <Field label="关联销售订单（可选）" htmlFor="engineering-order">
            <AsyncSelect
              ...
              value={orderId}
              reference={{ model: 'order', labelField: 'orderNo', valueField: 'id' }}
              onChange={setOrderId}
              placeholder="按订单号搜索"
            />
          </Field>
          ...
          <Button className="w-full" loading={busyKey === 'create'} onClick={() => void createDocument()}>
            创建草稿版本
          </Button>
        </div>
      </Sheet>
      ...
      >
        <div className="space-y-4">
          <FilePicker label="新版本文件" file={revisionFile} onChange={setRevisionFile} />
          ...
          <Button
            className="w-full"
            loading={Boolean(revisionTarget && busyKey === `${revisionTarget.id}:revision`)}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Button
              ...
              loading={Boolean(reviewTarget && busyKey === `${reviewTarget.id}:review`)}
...
function DocumentCard({
  ...
  const canAddRevision = latest && ['RELEASED', 'OBSOLETE', 'CHANGES_REQUESTED'].includes(latest.status);
  return (
    <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <header className="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className="font-mono text-sm font-bold text-blue-700">{document.documentNo}</span>
            <Badge status={latest?.status}>{statusLabels[latest?.status] ?? latest?.status}</Badge>
            <span className="text-xs text-slate-500">{typeLabels[document.documentType] ?? document.documentType}</span>
          </div>
          <h2 className="mt-1 truncate text-base font-semibold text-slate-950">{document.title}</h2>
          <p className="mt-1 text-xs text-slate-500">
            {document.product ? `${document.product.sku} · ${document.product.name}` : '未关联产品'}
        ...
        <div className="flex flex-wrap gap-2">
          {latest?.status === 'DRAFT' && can('engineeringDocument:create') ? (
            <Button size="sm" loading={busyKey === `${latest.id}:submit`} icon={<Send className="h-4 w-4" />} onClick={() => onSubmit(latest)}>
              提交校审
            </Button>
          ) : null}
          {latest?.status === 'PENDING_REVIEW' && can('engineeringDocument:review') ? (
            <Button size="sm" variant="info" onClick={() => onReview(latest)}>开始校审</Button>
          ) : null}
          {latest?.status === 'PENDING_APPROVAL' && can('engineeringDocument:approve') ? (
            <Button size="sm" loading={busyKey === `${latest.id}:release`} icon={<ShieldCheck className="h-4 w-4" />} onClick={() => onRelease(latest)}>
              批准发布
            </Button>
          ) : null}
          {canAddRevision && can('engineeringDocument:create') ? (
            <Button size="sm" variant="secondary" icon={<UploadCloud className="h-4 w-4" />} onClick={() => onAddRevision(document)}>
              上传新版本
            </Button>
          ) : null}
        </div>
      ...
      <div className="divide-y divide-slate-100">
        {document.revisions.map((revision) => (
          <div key={revision.id} className="grid gap-3 px-5 py-3 md:grid-cols-[100px_1fr_auto] md:items-center">
            <div>
              <p className="font-mono text-sm font-semibold text-slate-800">R{String(revision.revisionNo).padStart(2, '0')}</p>
              <Badge status={revision.status}>{statusLabels[revision.status] ?? revision.status}</Badge>
            </div>
            <div className="min-w-0">
              <p className="truncate text-sm font-medium text-slate-800">{revision.fileRecord.fileName}</p>
              <p className="mt-1 text-xs text-slate-500">
                设计：{revision.creator.name}
                {revision.reviewer ? ` · 校审：${revision.reviewer.name}` : ''}
                {revision.approver ? ` · 批准：${revision.approver.name}` : ''}
              </p>
              {revision.reviewComment ? <p className="mt-1 text-xs text-amber-700">意见：{revision.reviewComment}</p> : null}
              <p className="mt-1 truncate font-mono text-[10px] text-slate-400">SHA-256 {revision.checksumSha256}</p>
            </div>
            <Button size="sm" variant="ghost" icon={<Download className="h-4 w-4" />} onClick={() => onDownload(revision)}>
              下载
            </Button>
          </div>
        ))}
...
function Field({ label, htmlFor, children }: { label: string; htmlFor: string; children: React.ReactNode }) {
  return (
    <div>
      <label htmlFor={htmlFor} className="mb-1 block text-xs font-semibold text-slate-600">{label}</label>
      {children}
    </div>
  );
}
...
function FilePicker({ label, file, onChange }: { label: string; file: File | null; onChange: (file: File | null) => void }) {
  return (
    <label className="block cursor-pointer rounded-xl border border-dashed border-slate-300 bg-slate-50 p-5 text-center hover:bg-slate-100">
      ...
      <span className="mt-2 block text-sm font-medium text-slate-700">{file?.name ?? label}</span>
      <span className="mt-1 block text-xs text-slate-500">PDF、图片、Office 文档或文本文件</span>
      <input
        aria-label={label}
        type="file"
        className="hidden"
