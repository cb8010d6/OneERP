...
export default function ReceivingPage() {
  ...
  const qtyError = useMemo(() => {
    ...
    if (isNaN(num) || num <= 0) return '数量必须大于 0';
    if (num > selectedItem.remainingQty) return `超过可收货数量 (${selectedItem.remainingQty})`;
    return null;
    ...
    <div className="h-full space-y-5 bg-slate-50/40 p-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-500">
            选择采购单、库位，输入批次和数量完成收货入库。支持扫码枪快速收货。
          </p>
        </div>
      ...
      {scanMode && (
        <div className="rounded-xl border border-blue-200 bg-blue-50 p-4">
          <div className="mb-2 flex items-center gap-2 text-sm font-semibold text-blue-700">
            <Scan className="h-4 w-4" />
            扫码枪收货模式 – 扫入物料条码自动匹配最近采购单并收货 1 件
          </div>
          <div className="flex items-center gap-2">
            <input
              ref={scanInputRef}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="erp-card space-y-5 p-5">
              ...
              >
                ...
                {orders.map((o) => (
                  <option key={o.id} value={o.id}>
                    {o.orderNo} | {o.supplier.name} | {o.items.length} 项待收
                  </option>
                ))}
              </select>
          ...
          {selectedOrder && (
            <div>
              <label className="mb-1 block text-sm font-medium text-slate-700">物料明细</label>
              <div className="relative">
                <select
                ...
                >
                  ...
                  {selectedOrder.items.map((item) => (
                    <option key={item.id} value={item.id}>
                      {item.material.name} ({item.material.sku}) | 可收: {item.remainingQty} {item.material.unit}
                    </option>
                  ))}
                </select>
                <ChevronDown className="pointer-events-none absolute right-2.5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
          ...
          {selectedItem && (
            <div className="rounded-lg bg-slate-50 p-3 text-sm">
              ...
              <div className="flex items-center justify-between mt-1 border-t border-slate-200 pt-1">
                <span className="text-slate-500 font-medium">可收货</span>
          ...
          <div>
            ...
            <input
              ...
              placeholder={selectedItem ? `最大 ${selectedItem.remainingQty}` : '请输入数量'}
              min="0.01"
