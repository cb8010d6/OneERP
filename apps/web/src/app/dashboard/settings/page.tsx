...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    if (currentCompanyId) {
      ...
      setCompany(found ? { id: found.id, name: found.name } : { id: currentCompanyId, name: t('settingsCompany') });
    }
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="p-6">
          ...
          {activeTab === 'departments' && (
            <DynamicView modelName="department" title={t('settingsDepartments')} />
          )}

          {activeTab === 'info' && (
            <div className="max-w-2xl space-y-6">
              <div>
                <h3 className="text-lg font-bold mb-2 text-gray-900 border-b pb-2">{t('settingsCompanyInfo')}</h3>
                <div className="grid grid-cols-2 gap-6 mt-4">
                  <div className="space-y-1">
                    <label className="text-sm font-medium text-gray-600">{t('settingsActiveOrg')} (ID: {company?.id})</label>
                  ...
                  <div className="space-y-1">
                    <label className="text-sm font-medium text-gray-600">{t('settingsTaxId')}</label>
                    <input type="text" placeholder={t('settingsTaxPlaceholder')} className="w-full bg-white border border-gray-300 rounded p-2 text-gray-900 focus:ring-2 focus:ring-blue-500" />
                  </div>
                </div>
              </div>

              <div className="pt-4">
                <h3 className="text-lg font-bold mb-2 text-gray-900 border-b pb-2">{t('settingsPdaTitle')}</h3>
                <div className="bg-yellow-50 text-yellow-800 p-4 rounded-lg flex items-start border border-yellow-200">
                  <p className="text-sm">{t('settingsPdaHint')}<br/><br/><code>http://{localIp}:8000/api</code></p>
                </div>
              </div>
            </div>
          )}
