...
export class LlmAdapterService {
  ...
  async resolveReadSql(question: string, schemaContext: string): Promise<string | null> {
    const apiKey = process.env.OPENAI_API_KEY;
    if (!apiKey) {
      return null;
    }
    ...
    try {
      ...
      return content.replace(/^```sql\s*/i, '').replace(/^```\s*/i, '').replace(/```$/i, '').trim();
    } catch (error) {
      this.logger.warn(
  ...
  async resolveDocumentDraft(fileName: string): Promise<Record<string, unknown> | null> {
    const apiKey = process.env.OPENAI_API_KEY;
    if (!apiKey) {
      return null;
    }
