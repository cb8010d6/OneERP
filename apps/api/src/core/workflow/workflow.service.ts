...
export class WorkflowService {
  ...
  async transition(
    ...
    const workflow = await this.findOrBootstrapWorkflow(normalizedModel, companyId);
  ...
  private async findOrBootstrapWorkflow(modelName: string, companyId: string) {
    ...
    try {
      await this.prisma.$transaction(async (tx) => {
        ...
        const stateIdMap = new Map(states.map((item) => [item.value, item.id]));
  ...
  private findActiveWorkflow(modelName: string, companyId: string) {
