...
export class PurchaseService {
  ...
  async createPurchaseOrder(companyId: string, data: CreatePurchaseOrderDto) {
    ...
    let totalAmount = 0;
    for (const item of data.items) totalAmount += item.quantity * item.unitPrice;
  ...
  async getPurchaseOrderById(orderId: string, companyId: string) {
    ...
    const itemsWithRemaining = order.items.map((item) => ({
      ...item,
      remainingQty: Math.max(0, item.quantity - item.receivedQty),
    }));
  ...
  async updatePurchaseOrderStatus(
    ...
    if (!order) throw new NotFoundException('采购订单不存在或无权限访问');
    if (order.status === 'CANCELLED') throw new BadRequestException('已取消的采购单不能更新状态');
  ...
  async receivePurchaseItem(
    ...
    if (!order) throw new NotFoundException('采购订单不存在或无权限访问');
    if (order.status === 'CANCELLED') throw new BadRequestException('已取消的采购单不能收货');
    if (order.status === 'RECEIVED') throw new BadRequestException('采购单已全部收货完成');
    ...
    if (remainingQty <= 0) throw new BadRequestException('该明细已全部收货完成');
    if (data.quantity > remainingQty) {
      throw new BadRequestException(`收货数量(${data.quantity})超过可收货数量(${remainingQty})`);
    }
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      const newReceivedQty = orderItem.receivedQty + data.quantity;
  ...
  async scanReceive(
    companyId: string,
    data: { materialSku: string; quantity: number; destLocationId: string; batchNo?: string },
    operatorId: string,
  ) {
    ...
    const orderItem = await this.prisma.purchaseOrderItem.findFirst({
      where: {
        materialId: material.id,
        purchaseOrder: { companyId, status: { in: ['CONFIRMED', 'PARTIALLY_RECEIVED'] } },
      },
    ...
    const remainingQty = orderItem.quantity - orderItem.receivedQty;
    if (remainingQty <= 0) throw new BadRequestException('该采购明细已全部收货');
  ...
  async getPendingReceivableOrders(companyId: string) {
    ...
    return orders
      .map((order) => ({
        ...order,
        items: order.items.filter((item) => item.receivedQty < item.quantity),
      }))
