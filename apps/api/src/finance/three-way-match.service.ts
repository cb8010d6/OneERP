...
export class ThreeWayMatchService {
  ...
  private buildNotApplicableResult(
    invoiceId: string,
    invoiceNo: string,
  ): ThreeWayMatchResult {
    return {
  ...
  async validateMatch(
    ...
    toleranceAbs = DEFAULT_TOLERANCE_ABS,
    ...
    if (!po) {
      this.logger.warn(
        `入库单 ${receipt.receiptNo} 未关联采购订单，跳过三单匹配`,
      );
      return this.buildNotApplicableResult(invoice.id, invoice.invoiceNo);
    }
    ...
    for (const gl of receipt.lines) {
      grQtyByMat.set(
        gl.materialId,
        (grQtyByMat.get(gl.materialId) ?? 0) + gl.quantity,
      );
    }
    ...
    for (const bl of invoice.lines) {
      ...
      if (bl.quantity > poQty + toleranceAbs)
        reasons.push(`开票数量(${bl.quantity})超过PO采购数量(${poQty})`);
      if (
        poTotal > 0 &&
        Math.abs(amtDiff) > toleranceAbs &&
      ...
      lineDetails.push({
        materialId: matId,
        materialName: bl.material?.name,
        poQuantity: poQty,
        poUnitPrice: poPrice,
        poSubTotal: this.round2(poSub),
        poTaxAmount: this.round2(poTax),
    ...
    return {
      ...
      overallMatched: overall,
      lines: lineDetails,
      summary: {
        totalPoAmount: this.round2(totalPoAmount),
        totalGrQuantity: this.round2(totalGrQty),
        totalBillAmount: this.round2(totalBillAmount),
        totalBillTax: this.round2(totalBillTax),
        amountDifference: this.round2(totalBillAmount - totalPoAmount),
        taxDifference: this.round2(
          totalBillTax - lineDetails.reduce((s, l) => s + l.poTaxAmount, 0),
        ),
        mismatchedLineCount: mismatched,
        totalLineCount: lineDetails.length,
      },
      checkedAt: new Date(),
    };
  }
  ...
  async validateAndPersist(
    ...
    const result = await this.validateMatch(
      companyId,
      invoiceId,
      toleranceRate,
      toleranceAbs,
    ...
    await this.prisma.auditLog.create({
      data: {
        ...
        details: {
          ...
          overallMatched: result.overallMatched,
          summary: result.summary,
          mismatchedLines: result.lines
            .filter((l) => !l.matched)
            .map((l) => ({ materialId: l.materialId, reasons: l.reasons })),
        },
        companyId,
      },
    });
    if (!result.overallMatched) {
      const mismatchedReasons = result.lines
        .filter((l) => !l.matched)
        .flatMap((l) => l.reasons);
